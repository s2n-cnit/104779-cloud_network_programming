...
from error import (InternalServerErrorException, NotAcceptableException,
                   NotFoundException)
    ...
    def create(self: Self, model: ModelType, user: User) -> Result:
        try:
            ...
            raise NotAcceptableException(
                target=self.model_text, detail=str(ie)
            )
        except Exception as e:
            raise InternalServerErrorException(
                target=self.model_text, detail=str(e)
            )
    ...
    def read(self: Self, id: str | int) -> ModelType:
        with Session(engine) as session:
            try:
                ...
                raise InternalServerErrorException(
                    target=self.model_text, id=id, detail=str(e)
                )
            if not db:
